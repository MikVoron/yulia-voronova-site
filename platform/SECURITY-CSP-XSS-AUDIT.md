...
Оба retry-handler в шаблонах `data-v2.js` заменены общим
`data-shared-action="reload"`; cache-bust синхронно обновлён во всех девяти
...
Третья партия завершена для `login.html` и `login.js`: CSS-блок вынесен в
`login.css`, два статических атрибута и динамические `element.style` / inline
шаблон ошибки заменены семантическими классами. Desktop/mobile production-smoke
обычного и admin/MFA-входа подтвердил совпадение геометрии и видимых состояний,
