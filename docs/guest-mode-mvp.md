...
**Автор:** Михаил + Claude
**Дата:** 2026-05-11
**Цель документа:** описать что и как меняется, чтобы незарегистрированный пользователь мог зайти на `app.voronova.online` и увидеть каталог рецептов с понятной ценностью продукта.
...
- **Категория рецепта** — это про *тип блюда* (`breakfasts`, `mains`, `salads`, `spreads`, `sauces`, `drinks`, ...). Без изменений.
...
**Preview-state показывает:**
- Фото, название, КБЖУ, время, сложность, теги, цитату (`quote`).

**Вместо ингредиентов / шагов / заметки** — один CTA-блок. Содержимое берётся из `Auth.recipePaywallCta(recipe)`:
...
API уже отдаёт обрезанные данные (без `ingredients/steps/note`) для пользователей без доступа — фронту нужно только корректно отрисовать preview-state, **не пытаясь** показывать пустые секции.
...
- `robots.txt` на `app.voronova.online` — разрешить индексацию каталога, запретить `cabinet.html`, `admin*`, `login.html`.
...
- **Расширить `/content/recipes`** — отдавать гостю короткий «тизер» (1-2 первых шага) у платных рецептов? Сейчас режется всё. **Решение:** в первой итерации не трогаем.
...
2. `robots.txt` и `sitemap.xml` на `app.voronova.online`.
...
- [ ] Открываем `app.voronova.online` → главная **не редиректит** на login.
...
- [ ] Клик на paid-рецепт → preview-state: фото, КБЖУ, цитата + CTA-блок «Зарегистрироваться». **Никаких пустых блоков** ингредиентов/шагов.
...
2. **Что показывать в preview платного рецепта на `recipe.html`?** Решено (§6.4): фото, название, КБЖУ, время, сложность, теги, цитата. Шагов и ингредиентов — **не показывать вообще**.
...
- **Посещения `app.voronova.online`** (раньше = только зарегистрированные, теперь — все) — должно вырасти в 5-20x в зависимости от трафика.
