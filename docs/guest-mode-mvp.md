...
- Первые 4 ингредиента и текст первого шага без фотографий шагов.
...
- **Расширить `/content/recipes`** — реализовано: гостю отдаются 4 строки состава и текст первого шага у `trial`/`pro`; полный состав, остальные шаги, заметка и фотографии шагов остаются закрыты.
...
- [ ] Клик на paid-рецепт → подробный preview-state: фото, КБЖУ, цитата, 4 ингредиента, первый шаг и корректный CTA для уровня доступа.
...
2. **Что показывать в preview платного рецепта на `recipe.html`?** Решено (§6.4): фото, название, КБЖУ, время, цитату, 4 ингредиента и первый шаг без фотографии; остальной контент закрыт.
