...
describe('optional protein add-on for soups', () => {
  ...
  it('shows optional protein but excludes it from balance gating', () => {
    ...
    expect(recipePage).toContain('const balanceGroupOrder = { p: 0, f: 1, c: 2, fi: 3 };');
    expect(recipePage).toContain('_balCats.forEach((c, i) => { c.stepIndex = i; });');
    expect(recipePage).toContain("_balCats.map(c => buildGroup(c.prefix, addItemsByPrefix[c.prefix], stepOf[c.prefix])).join('')");
