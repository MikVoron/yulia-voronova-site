...
До допуска к production ещё нужны: атомарное устойчивое сохранение связанной
пары state/recovery (например, общим защищённым envelope), журнал намерения перед
запуском таймера, обработка обрыва между подготовкой и `arm`, защищённый boot
