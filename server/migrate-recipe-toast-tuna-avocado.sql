...
ON CONFLICT (id) DO UPDATE SET
  ...
  steps = jsonb_build_array(
    jsonb_strip_nulls(jsonb_build_object('text', CASE WHEN jsonb_typeof(EXCLUDED.steps->0) = 'object' THEN EXCLUDED.steps->0->>'text' ELSE EXCLUDED.steps->>0 END, 'photo', 'images/recipes/toast-tuna-avocado/toast-tuna-avocado-1.webp')),
    jsonb_strip_nulls(jsonb_build_object('text', CASE WHEN jsonb_typeof(EXCLUDED.steps->1) = 'object' THEN EXCLUDED.steps->1->>'text' ELSE EXCLUDED.steps->>1 END)),
    jsonb_strip_nulls(jsonb_build_object('text', CASE WHEN jsonb_typeof(EXCLUDED.steps->2) = 'object' THEN EXCLUDED.steps->2->>'text' ELSE EXCLUDED.steps->>2 END, 'photo', 'images/recipes/toast-tuna-avocado/toast-tuna-avocado-3.webp')),
    jsonb_strip_nulls(jsonb_build_object('text', CASE WHEN jsonb_typeof(EXCLUDED.steps->3) = 'object' THEN EXCLUDED.steps->3->>'text' ELSE EXCLUDED.steps->>3 END, 'photo', 'images/recipes/toast-tuna-avocado/toast-tuna-avocado-4.webp')),
    jsonb_strip_nulls(jsonb_build_object('text', CASE WHEN jsonb_typeof(EXCLUDED.steps->4) = 'object' THEN EXCLUDED.steps->4->>'text' ELSE EXCLUDED.steps->>4 END, 'photo', 'images/recipes/toast-tuna-avocado/toast-tuna-avocado-5.webp')),
    jsonb_strip_nulls(jsonb_build_object('text', CASE WHEN jsonb_typeof(EXCLUDED.steps->5) = 'object' THEN EXCLUDED.steps->5->>'text' ELSE EXCLUDED.steps->>5 END))
  ),
