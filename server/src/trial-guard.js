...
async function tryGrantTrial(fingerprint, ip, userId) {
  ...
  try {
    ...
    // 1. Один fingerprint может получить автоматический триал только один раз.
    // IP остаётся наблюдаемым сигналом, но смена сети/VPN не должна обходить
    // ограничение. Спорные совпадения разбираются вручную через поддержку.
    if (fingerprint) {
    ...
    // 2. Триал одобрен. Уникальная запись fingerprint фиксирует его первый триал.
    await client.query(
