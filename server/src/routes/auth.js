...
async function authRoutes(fastify) {
  ...
  fastify.post('/auth/send-code', async (req, reply) => {
    ...
    await db.query("INSERT INTO login_codes (email, code_hash, expires_at) VALUES ($1, $2, now() + interval '5 minutes')", [lower, codeHash]);
  ...
  fastify.post('/auth/verify', async (req, reply) => {
    ...
    reply.setCookie('refreshToken', refreshToken, {
      path: '/', httpOnly: true, secure: true, sameSite: 'none', maxAge: 2592000
    });
